...
import { AccountData, defaultAccountData, Pot, HistoryEntry, ScheduledWithdrawal, SplitRule } from '@/types/money-pots';
...
export function useAccountStore() {
  ...
  // Setup
  const completeSetup = useCallback((totalBalance: number, interestRate: number, pots: { name: string; balance: number }[]) => {
    update(d => ({
      ...d,
      totalBalance,
      interestRate,
      pots: pots.map(p => ({ id: uid(), name: p.name, balance: p.balance, history: [{ id: uid(), date: new Date().toISOString(), type: 'manual' as const, amount: p.balance, description: 'Initial balance' }] })),
      setupComplete: true,
    }));
  }, [update]);
  ...
  const addPot = useCallback((name: string, balance: number) => {
    update(d => ({
      ...
      pots: [...d.pots, { id: uid(), name, balance, history: [{ id: uid(), date: new Date().toISOString(), type: 'manual' as const, amount: balance, description: 'Initial balance' }] }],
    }));
  }, [update]);
  ...
  const updatePot = useCallback((potId: string, name: string, balance: number) => {
    update(d => {
      ...
      return {
        ...
        pots: d.pots.map(p => p.id === potId ? { ...p, name, balance, history: [...p.history, { id: uid(), date: new Date().toISOString(), type: 'manual' as const, amount: diff, description: 'Balance adjusted' }] } : p),
      };
    });
  }, [update]);
    ...
    update(d => {
      ...
      for (const pot of newPots) {
        ...
        if (interest > 0) {
          ...
          pot.history.push({ id: uid(), date: now, type: 'interest', amount: interest, description: `Interest (${d.interestRate}% p.a.)` });
        }
      }
    ...
    update(d => {
      ...
      return {
        ...
        pots: d.pots.map(p => p.id === w.potId ? {
          ...
          history: [...p.history, { id: uid(), date: now, type: 'withdrawal' as const, amount: -w.amount, description: w.description }],
        } : p),
