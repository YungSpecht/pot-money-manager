                                ...
                                <div key={w.id} className="flex items-center justify-between p-3 rounded-lg bg-muted">
                                    <div>
                                        ...
                                        <p className="text-xs text-muted-foreground">
                                            {data.pots.find(p => p.id === w.potId)?.name} ·
                                            Day {w.dayOfMonth} · {w.recurring ? 'Recurring' : 'One-time'}
                                        </p>
                                    </div>
                    ...
                    <div className="space-y-3 border-t pt-4">
                        ...
                        <div className="flex gap-2">
                            <Input type="number" placeholder="Amount" value={amount}
                                   onChange={e => setAmount(e.target.value)} inputMode="decimal" className="flex-1"/>
                            <Input type="number" placeholder="Day" value={day} onChange={e => setDay(e.target.value)}
                                   min={1} max={31} className="w-20"/>
                        </div>
                        <div className="flex items-center justify-between">
                            <label className="text-sm">Recurring monthly</label>
                            <Switch checked={recurring} onCheckedChange={setRecurring}/>
                        </div>
                        <Button className="w-full" onClick={add} disabled={!potId || !parseFloat(amount)}>Add
                            Withdrawal</Button>
                    </div>
                </div>
            </SheetContent>
