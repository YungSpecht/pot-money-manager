...
export function SetupScreen({ onComplete }: SetupScreenProps) {
  ...
  const handleSubmit = () => {
    ...
    onComplete(balance, parseFloat(interestRate) || 0, validPots.map(p => ({ name: p.name.trim(), balance: parseFloat(p.balance) || 0 })));
  };
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                ...
                <Input type="number" placeholder="0.00" value={totalBalance} onChange={e => setTotalBalance(e.target.value)} inputMode="decimal" />
              </div>
              <div className="space-y-2">
                <label className="text-sm font-medium">Annual Interest Rate (%)</label>
                <Input type="number" placeholder="2.5" value={interestRate} onChange={e => setInterestRate(e.target.value)} inputMode="decimal" step="0.01" />
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {pots.map((pot, i) => (
                <div key={i} className="flex gap-2 items-start">
                  <div className="flex-1 space-y-1">
                    <Input placeholder="Pot name" value={pot.name} onChange={e => updatePot(i, 'name', e.target.value)} />
                  </div>
                  <div className="w-28">
                    <Input type="number" placeholder="0.00" value={pot.balance} onChange={e => updatePot(i, 'balance', e.target.value)} inputMode="decimal" />
                  </div>
                  {pots.length > 1 && (
              ...
              <div className="flex gap-2">
                ...
                <Button onClick={handleSubmit} disabled={overBudget || pots.every(p => !p.name.trim())} className="flex-1">
                  Get Started
                </Button>
              </div>
            </CardContent>
