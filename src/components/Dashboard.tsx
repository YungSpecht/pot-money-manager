...
import { Settings, Plus, Calendar, ArrowDownUp, TrendingUp, Wallet, PiggyBank } from 'lucide-react';
import { AccountData, SplitRule, ScheduledWithdrawal } from '@/types/money-pots';
  ...
  if (pot) {
    return (
      <PotDetail
        ...
        onUpdate={(name, balance) => { props.updatePot(pot.id, name, balance); }}
        onDelete={() => { props.deletePot(pot.id); setSelectedPot(null); }}
      />
    );
  }
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="bg-primary text-primary-foreground px-5 pt-12 pb-8 rounded-b-3xl">
        <div className="flex items-center justify-between mb-6">
          ...
          <Button variant="ghost" size="icon" className="text-primary-foreground hover:bg-primary-foreground/10" onClick={() => setShowSettings(true)}>
      ...
      <div className="px-5 -mt-4 space-y-4">
        ...
        {/* Pots */}
        <div>
          <h2 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-3">Your Pots</h2>
          <div className="space-y-2">
            {data.pots.map(p => {
              const pct = data.totalBalance > 0 ? (p.balance / data.totalBalance) * 100 : 0;
              return (
                <Card key={p.id} className="cursor-pointer hover:shadow-md transition-shadow active:scale-[0.99]" onClick={() => setSelectedPot(p.id)}>
            ...
            {data.pots.length === 0 && (
              <p className="text-center text-sm text-muted-foreground py-8">No pots yet. Tap + to add one.</p>
            )}
          </div>
        </div>
                ...
                return (
                  <Card key={w.id} className={due ? 'border-destructive/50' : ''}>
                    <CardContent className="flex items-center justify-between py-3 px-4">
                      ...
                      <div className="flex items-center gap-2">
                        <span className="font-semibold text-sm text-destructive">-€{w.amount.toFixed(2)}</span>
                        {due && (
                          <Button size="sm" variant="destructive" onClick={() => props.processWithdrawal(w.id)}>Due</Button>
                        )}
                      </div>
                    </CardContent>
                  </Card>
